...
func main() {
	...
	router.Handle("/callback_success", oauth.CallbackSuccessHandler()).Methods("GET")
	router.NewRoute().Path(oauth2.CallBackRoutePath).Queries("error", "", "error_description", "").Handler(oauth.CallbackErrorHandler())
