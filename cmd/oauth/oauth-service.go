...
import (
	...
	oauth2 "github.com/redhat-appstudio/service-provider-integration-operator/oauth"
...
func main() {
	args := oauth2.OAuthServiceCliArgs{}
	arg.MustParse(&args)
	...
	cfg, err := oauth2.LoadOAuthServiceConfiguration(args)
	if err != nil {
	...
	cl, err := oauth2.CreateClient(kubeConfig, client.Options{
		Mapper: mapper,
	})
	...
	tokenUploader := oauth2.SpiTokenUploader{
		K8sClient: cl,
	...
	authenticator := oauth2.NewAuthenticator(sessionManager, cl)
	stateStorage := oauth2.NewStateStorage(sessionManager)
	//static routes first
	router.HandleFunc("/health", oauth2.OkHandler).Methods("GET")
	router.HandleFunc("/ready", oauth2.OkHandler).Methods("GET")
	router.HandleFunc("/callback_success", oauth2.CallbackSuccessHandler).Methods("GET")
	router.HandleFunc("/login", authenticator.Login).Methods("POST")
	router.NewRoute().Path("/{type}/callback").Queries("error", "", "error_description", "").HandlerFunc(oauth2.CallbackErrorHandler)
	router.NewRoute().Path("/token/{namespace}/{name}").HandlerFunc(oauth2.HandleUpload(&tokenUploader)).Methods("POST")
	router.NewRoute().Path("/token/{kcpWorkspace}/{namespace}/{name}").HandlerFunc(oauth2.HandleUpload(&tokenUploader)).Methods("POST")
	...
	for _, sp := range cfg.ServiceProviders {
		...
		controller, err := oauth2.FromConfiguration(cfg, sp, authenticator, stateStorage, cl, strg, redirectTpl)
		if err != nil {
	...
	server := &http.Server{
		...
		Handler:           sessionManager.LoadAndSave(oauth2.MiddlewareHandler(strings.Split(args.AllowedOrigins, ","), router)),
	}
...
func kubernetesConfig(args *oauth2.OAuthServiceCliArgs) (*rest.Config, error) {
	if args.KubeConfig != "" {
