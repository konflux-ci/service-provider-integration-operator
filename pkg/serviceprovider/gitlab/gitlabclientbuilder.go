...
func (builder *gitlabClientBuilder) CreateAuthenticatedClient(ctx context.Context, token *oauth2.Token) (*gitlab.Client, error) {
	if token == nil {
		return nil, tokenNilError
	}
	client, err := gitlab.NewOAuthClient(token.AccessToken, gitlab.WithHTTPClient(builder.httpClient), gitlab.WithBaseURL(builder.gitlabBaseUrl))
	if err != nil {
