...
type ServiceAccountSpec struct {
	// LinkSecretAs specifies how the secret generated by the binding is linked to the service account.
	// This can be either `secret` meaning that the secret is listed as one of the mountable secrets
	// in the `.secrets` of the service account, or `imagePullSecret` which makes the secret listed as
	// one of the image pull secrets associated with the service account. If not specified, it defaults
	// to `secret`.
	// +optional
	// +kubebuilder:default:=secret
	LinkSecretAs SecretLinkType `json:"linkSecretAs"`
...
type SecretLinkType string

const (
	SecretLinkTypeSecret          SecretLinkType = "secret"
	SecretLinkTypeImagePullSecret SecretLinkType = "imagePullSecret"
)
...
func (in *SPIAccessTokenBinding) Validate() SPIAccessTokenBindingValidation {
	...
	if in.Spec.ServiceAccount.LinkSecretAs == SecretLinkTypeImagePullSecret && in.Spec.Secret.Type != corev1.SecretTypeDockerConfigJson {
		ret.Consistency = append(ret.Consistency,
			fmt.Sprintf("the secret must have the %s type for it to be linkable to the specified service account as an image pull secret", corev1.SecretTypeDockerConfigJson))
	}
...
// the user.
func (s *ServiceAccountSpec) EffectiveSecretLinkType() SecretLinkType {
	if s.LinkSecretAs == SecretLinkTypeImagePullSecret {
		return SecretLinkTypeImagePullSecret
	}
	return SecretLinkTypeSecret
}
