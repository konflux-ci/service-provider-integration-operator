...
func (s *SpiTokenFetcher) BuildHeader(ctx context.Context, namespace, repoUrl string, loginCallback func(ctx context.Context, url string)) (*HeaderStruct, error) {
	...
	for timeout := time.After(duration); ; {
		...
		if err != nil {
			lg.Error(err, "Error reading TB item")
		}
		...
		case <-timeout:
			lg.Error(err, "Timeout reached reading TB item")
			return nil, fmt.Errorf("reading the token binding %w", timeoutError)
		default:
			time.Sleep(200 * time.Millisecond)
		}
	}
	lg.Info("Watching Access Token to fetch file", "token", tokenName)
	...
	for timeout := time.After(10 * duration); ; {
		...
		if err != nil {
			lg.Error(err, "Error reading access token")
		}
		...
		if readToken.Status.Phase == v1beta1.SPIAccessTokenPhaseAwaitingTokenData && !loginCalled {
			...
			lg.Info("calling callback with URL to OAUth", "url", url)
		...
		case <-ctx.Done():
			return nil, taskCancelledError
		case <-timeout:
			lg.Error(err, "Timeout reached reading Token item.")
	...
	lg.Info("Watching the credentials secret", "secret", secretName)
	...
	for timeout := time.After(duration); ; {
		...
		case <-ctx.Done():
			return nil, taskCancelledError
		case <-timeout:
			lg.Error(err, "Timeout reached reading Secret")
