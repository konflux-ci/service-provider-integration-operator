...
test: manifests generate fmt vet envtest ## Run unit tests
	GOMEGA_DEFAULT_EVENTUALLY_TIMEOUT=3s KUBEBUILDER_ASSETS="$(shell $(ENVTEST) --arch=amd64 use $(ENVTEST_K8S_VERSION) -p path)" go test ./... -coverprofile cover.out -covermode=atomic -coverpkg=./...
